...
/**
 * Describes which Xposed module pipeline an app belongs to.
 */
...
/**
 * Immutable snapshot of the Xposed module metadata of an APK.
 */
@Parcelize
...
/**
 * Reads Xposed module metadata from installed packages or raw APK files.
 *
 * Modern modules ship a `META-INF/xposed/module.prop` plus optional
 * scope/init lists, while legacy modules rely on manifest meta-data and
 * `assets/xposed_init`.
 */
object ModuleMetadataReader {
    ...
    /**
     * Reads module metadata for an installed package.
     *
     * Returns null when the package is not a module, is a patched app
     * (marked with the `npatch` meta-data key), or the APK cannot be read.
     */
    fun read(packageInfo: PackageInfo, packageManager: PackageManager): ModuleMetadataSnapshot? {
        ...
        // Patched apps must never be treated as modules.
        ...
        val hasLegacyMetadata =
            legacyMeta?.containsKey(LEGACY_KEY_MIN_API_VERSION) == true ||
                legacyMeta?.containsKey(LEGACY_KEY_DESCRIPTION) == true
    ...
    /**
     * Reads metadata from an APK file that is not necessarily installed.
     */
    ...
    private fun readString(properties: Properties, key: String): String? =
